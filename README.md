# Angular Starter Card

The Angular Starter Card is a boilerplate project for building Dome cards using Angular.
...
1. Clone the `card-angular_starter` project.
...
### Angular Change Detection & Card SDK Callbacks

Card SDK lifecycle callbacks (such as `onInit`) are executed **outside of Angular’s zone**.
Because of this, Angular does **not** automatically trigger change detection for state updates made inside these callbacks.

To ensure the UI updates correctly, any state mutations performed inside SDK callbacks **must be wrapped in `ngZone.run(...)`**.
...
onInit: (data: any) => {
  ...
  // Card SDK callbacks run outside Angular's zone.
  // Wrap updates in `ngZone.run` to trigger change detection.
  this.ngZone.run(() => {
    this.user.set(user);
    this.uiPref.set(ui);
